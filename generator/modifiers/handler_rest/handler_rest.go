...
func (d *handlerRest) AppendInterfaceAndFunction() (err error) {
	path := d.Location + "/src/handler/rest/" + d.EntityNameSnakeCase + ".go"
...
func (d *handlerRest) getFunction(path string) (lines []string, err error) {
	f, err := os.OpenFile(path, os.O_RDWR, 0644)
	if err != nil {
		return nil, err
	}
